...
export default function OptionsModalPrizeWheel({
  ...
  async function handleBackgroundUpload(file: File) {
    try {
      ...
      const filePath = `host_${hostId}/prizewheel_${event.id}/background-${Date.now()}-${file.name}`;
  ...
  return (
    <Modal isOpen={true} onClose={onClose}>
      <div className={cn('text-white', 'space-y-4')}>
        <h2 className={cn('text-2xl', 'font-bold', 'text-center', 'mb-4')}>
          ⚙ Prize Wheel Options
        </h2>
        ...
        <div>
          ...
          <input
            ...
            className={cn('w-full', 'p-2', 'text-black', 'rounded')}
          />
        </div>
        ...
        {/* SPIN SPEED */}
        <div>
          <label className={cn('block', 'text-sm', 'font-semibold', 'mb-1')}>Spin Speed</label>
          <select
            value={spinSpeed}
        ...
        {/* BACKGROUND */}
        <div>
          <label className={cn('block', 'text-sm', 'font-semibold', 'mb-1')}>Background</label>
          <input
            type="text"
            value={bgValue}
            onChange={(e) => setBgValue(e.target.value)}
            placeholder="linear-gradient(...) or image URL"
            className={cn('w-full', 'p-2', 'text-black', 'rounded', 'mb-2')}
          />
          <label className={cn('block', 'text-xs', 'mb-1')}>Upload background image:</label>
          <input
            type="file"
            accept="image/*"
            onChange={(e) =>
              e.target.files && handleBackgroundUpload(e.target.files[0])
            }
            className={cn('w-full', 'text-sm', 'text-gray-300')}
            disabled={uploading}
          />
          {uploading && (
